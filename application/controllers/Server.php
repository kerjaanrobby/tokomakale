...
class Server extends CI_Controller {
    ...
    public function index()
	{
		...
		if (($status!="")&&($logged == "yes")&&($rol=="Superadmin")) {
			...
			$this->load->view("content/server", $data);
		} else {
			redirect('keluar');
		}
    }
    public function datalist()
    {
		...
		{
			$list = $this->mod_server->get_datatables();
		    $data = array();
		    $no = $_POST['start'];
		    foreach ($list as $l) {
			  	$no++;
		      	...
		      	$row[] = $l->namaserver;
		      	$row[] = $l->namagame;
				$row[] = "<button class='btn btn-warning btn-sm' onclick='editData(".$l->id.")'><i class='fa fa-edit'></i> Edit</button>
                        <button class='btn btn-danger btn-sm' onclick='hapusData(".$l->id.")'><i class='fa fa-trash'></i> Hapus</button>";
		      	$data[] = $row;
		    }
		    $output = array(
		     "draw" => $_POST['draw'],
		     "recordsTotal" => $this->mod_server->count_all(),
		     "recordsFiltered" => $this->mod_server->count_filtered(),
		     "data" => $data,
		    );
    ...
    public function datasimpan()
    {
		...
		if(($status!="")&&($logged=="yes"))
		{
			...
			if($id<>""){
				$where['id']=$id;
				if($this->mod_conf->UpdateData('servergame',$datains,$where)){
					$output['hasil']=1;
				...
				if($this->mod_conf->InsertData('servergame', $datains))
				{
					$output['hasil']=1;
    ...
    public function datadetail()
	{
		...
		if(($status!="")&&($logged=="yes"))
		{
			...
			$wharr = array("id"=>$id);
			$dtrol=$this->mod_server->listing($wharr);
    ...
    public function datahapus()
	{
		...
		if(($status!="")&&($logged=="yes"))
		{
			...
			$delrol = $this->mod_conf->DeleteData("servergame",$where);
			if($delrol){
				$output['hasil']=1;
