...
<!-- CONTENT -->
        ...
        <div class="container-fluid">
            <div class="row page-titles">
                <div class="col-md-5 align-self-center">
                    <h4 class="text-themecolor">Data Banner</h4>
                    </div>
                        ...
                        <div class="card">
                            <div class="card-body">
                                ...
                                <div class="table-responsive m-t-40">
                                    <table id="tablebanner" class="table table-bordered table-striped" cellspacing="0" width="100%">
                                        <thead>
                                            <tr>
                                                <th>No</th>
                                                <th>Nama</th>
                        ...
                        <div class="row">
                            <div class="col-sm-12">
                                ...
                                <div>
                                    ...
                                    <input type="text" name="tglmulai" id="tglmulai" class="form-control"  required />
                                </div>
                                <div>
                                    <label>Tanggal Selesai</label>
                                    <input type="text" name="tglselesai" id="tglselesai" class="form-control"  required />
                                </div>
                                <div>
                                    <label>Selalu Aktif</label>
                                    <select class="form-control" name="alwayson" id="alwayson" required>
    ...
    <script>
        jQuery('#tglmulai').datepicker({
            format: 'dd/mm/yyyy',
        });
        jQuery('#tglselesai').datepicker({
            format: 'dd/mm/yyyy',
        });
        var modal = $("#modalform");
        ...
        function resetform(){
            $('#id').val("");
            $("#imgbanner").attr('src','<?php echo base_url();?>assets/image/imgdef.jpeg');
            form.reset();
        };
        function showData(){
            ...
            dttable = table.DataTable({
              ...
              order: [],
              ajax: {
                url: "<?php echo site_url('banner/datalist')?>",
                type: "POST"
              },
              "columnDefs": [{
        ...
        function editData(id){
            ...
            $.ajax({
                type: "POST",
                url: '<?php  echo base_url(); ?>banner/datadetail',
                data:{
                id:id
              },
              dataType:'json',
              success: function(data) {
                $.each(data.hasildata,function(i,v){
                    $('#id').val(v.idbanner);
                    $('#nama').val(v.nama);
                    $('#tglmulai').val(v.tglmulaif);
                    $('#tglselesai').val(v.tglselesaif);
                    $('#alwayson').val(v.alwayson);
                    $('#url').val(v.url);
                    if(v.icon!=""){
                        $("#imgbanner").attr('src','<?php echo base_url();?>assets/image/'+v.banner);
                    }
                });
              }
            });
        ...
        function hapus(){
            ...
            $.ajax({
                url: "<?php  echo base_url(); ?>banner/datahapus",
                type: "POST",
                data:  {id,id},
                dataType:'json', 
                success: function(data)
        ...
        $(function () {
            $('#formInput').on('submit', function (e) {
                e.preventDefault();
                $.ajax({
                    url: "<?php  echo base_url(); ?>banner/datasimpan",
                    type: "POST",
