...
<!-- CONTENT -->
        ...
        <div class="container-fluid">
            <div class="row page-titles">
                <div class="col-md-5 align-self-center">
                    <h4 class="text-themecolor">Jadi Reseller</h4>
                    </div>
                    <div class="col-md-7 align-self-center text-right">
                        <div class="d-flex justify-content-end align-items-center">
                            <ol class="breadcrumb">
                                <li class="breadcrumb-item"><a href="javascript:void(0)">Jadi Reseller</a></li>
                            </ol>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="card">
                            <form action=""  enctype="multipart/form-data" id="formInput" method="POST" class="form-horizontal" role="form">
                                <div class="card-body">
                                    ...
                                    <div>
                                        <label>Syarat Reseller</label>
                                        <textarea class="textarea_editor form-control" rows="15" name="reseller" id="reseller"  placeholder="Enter text ..."></textarea>
                                    </div>
                                    <div>
                                        <label>Syarat Reseller VIP</label>
                                        <textarea class="textarea_editor form-control" rows="15" name="resellervip" id="resellervip"  placeholder="Enter text ..."></textarea>
    ...
    <script>
        ...
        function showData(){
            $.ajax({
                type: "POST",
                url: '<?php  echo base_url(); ?>daftarreseller/datadetail',
                dataType:'json',
                success: function(data) {
                    $.each(data.hasildata,function(i,v){
                        $('#id').val(v.idsyaratreseller);
                        // console.log(v.reseller);
                        var editorObj = $("#reseller").data('wysihtml5');
                        var editor = editorObj.editor;
                        editor.setValue(v.reseller);

                        var editorObjvip = $("#resellervip").data('wysihtml5');
                        var editorvip = editorObjvip.editor;
                        editorvip.setValue(v.resellervip);
                        // $('#resellervip').contents().find('body').html(v.resellervip); 
                    },
                    );
                }
            });
        ...
        $(function () {
            $('#formInput').on('submit', function (e) {
                e.preventDefault();
                $.ajax({
                    url: "<?php  echo base_url(); ?>Daftarreseller/datasimpan",
                    type: "POST",
